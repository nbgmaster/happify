							            ...
							            <div style="display:table-row;">           	
							            	...
							            	<div style="display:table-cell;vertical-align:middle;text-align:center;padding:6px;width:230px">
						            			...
						            			<div id="radio_{$result.ID}" style="padding-left:0;margin-left:0; width:100%;vertical-align:middle;background-color:#e6efea;padding-top:4px">
								        			<input type="radio" id="radio{$count2}" value="2" name="{$result.ID}" /><label for="radio{$count2}">&nbsp;</label>
													<input type="radio" id="radio{$count3}" value="1" name="{$result.ID}" /><label for="radio{$count3}">&nbsp;</label>
													<input type="radio" id="radio{$count4}" value="0" name="{$result.ID}" /><label for="radio{$count4}">&nbsp;</label>
													<input type="radio" id="radio{$count5}" value="-1" name="{$result.ID}" /><label for="radio{$count5}">&nbsp;</label>
													<input type="radio" id="radio{$count6}" value="-2" name="{$result.ID}" /><label for="radio{$count6}">&nbsp;</label>
								        	    </div>    
...
$(document).ready(function(){
  ...
  /*	if (!$("input:radio[name='b1']:checked").val()) {
      ...
      return xajax_insertdata('{$tbl_da_scale_results}', xajax.getFormValues('insert'));
      //return false;
    //}
  });
});
